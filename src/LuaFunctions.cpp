...
int LuaInterpreter_setAgentPosition(lua_State *L)
{
  int agentNum = luaL_checkinteger(L,2);
  // Vector3 check defined in luawrapperutil
  RVO::Vector3 target = luaU_check<RVO::Vector3>(L,3);

  Flock::m_lua->setAgentPosition(agentNum,target);
  return 0;
}
...
int LuaInterpreter_compareVectors(lua_State *L)
{
  ...
  lua_pushnumber(L,Flock::m_lua->compareVectors(firstVector,secondVector));
  return 1;
}
...
{ 
  ...
  { "setAgentPosition", LuaInterpreter_setAgentPosition },
